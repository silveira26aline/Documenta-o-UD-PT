...
\section{Verbo \textit{ser} como voz passiva}\label{sec:servozpassiva}
...
\section{Verbos auxiliares}\label{sec:verbosauxiliares}
	...
	Verbos auxiliares estão classificados como \emph{AUX}. O que conta como um verbo auxiliar é alvo de discussão nas gramáticas do português (\citet{elvis2019locverbal}). De modo geral, classificamos como verbos auxiliares, além dos tempos compostos (\fullref{sec:auxtempocomposto}), as locuções verbais aspectuais (\fullref{sec:auxaspecto}) e modais (\fullref{sec:auxmodal}).
	
	Via de regra, verbos auxiliares (\emph{AUX}) não podem ter dependentes, e dependem de um verbo principal (\emph{VERB}), tendo deprel também \emph{AUX} (diferentemente dos verbos de ligação (\fullref{sec:verbosdeligação}) \say{ser} e \say{estar}, que têm deprel \emph{cop}, e do verbo \say{ser} como voz passiva (\fullref{sec:servozpassiva}), que tem deprel \emph{aux:pass}).
	
		\subsection{Locuções verbais de tempo composto}\label{sec:auxtempocomposto}
	
			Segundo as gramáticas, são locuções verbais de tempo composto aquelas que têm como verbo auxiliar \say{ter}, \say{haver} e, para nós, também \say{ir}. Confira um exemplo de locução verbal de tempo composto na \fullref{dep:auxtempocomposto}. Repare que não há nenhuma diferença entre a anotação dessa locução e uma locução verbal aspectual, de modo que a \fullref{tab:aux} mostra a lista de verbos auxiliares de locuções verbais tanto de tempo composto quanto aspectuais.
		...
		\subsection{Locuções verbais aspectuais/modais}\label{sec:auxaspecto}
			...
			Confira um exemplo de locução verbal aspectual na \fullref{dep:auxphrasalverb}, e modal na \fullref{dep:auxmodal}. Repare que não há nenhuma diferença entre a anotação dessas locuções e uma locução verbal de tempo composto (o primeiro verbo tem upos \emph{AUX} e depende do verbo principal, \emph{VERB}), de modo que a \fullref{tab:aux} mostra a lista de colocações de locuções verbais tanto de tempo composto quanto aspectuais/de modo.
			...
			Repare que, nos casos de locução verbal aspectual, geralmente há uma partícula interveniente no meio da locução, como na \fullref{dep:auxphrasalverb}. Encaramos que estamos diante de um fenômeno de \emph{phrasal verb}, uma MWE do tipo \emph{AUX}, como sugerido em \citet{elvis2019locverbal}. Na frase, \emph{vai ouvir} é uma locução verbal de tempo composto (\fullref{sec:auxtempocomposto}), e \emph{começar a ouvir} é uma locução verbal aspectual. Listamos essas colocações com partículas intervenientes na \fullref{tab:auxphrasalverb}.
