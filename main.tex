...
\section{Sujeito da oração}

	\subsection{Caso seja nominal}
		O sujeito será anotado com a deprel \emph{nsubj}. (ver Xx - nsubj:pass)
	...
	Em construções como \fullref{dep:serVERB}, não se considera \say{haver} como \emph{csubj}, mas como \emph{ccomp}, uma vez que o verbo \say{ser} é pleno (\fullref{sec:serpleno}).

\section{Adjuntos do verbo}

	\subsection{Adjunto adverbial}

		\subsubsection{Caso seja advérbio}
			...
			\begin{figure*}[htbp]
				...
				\begin{dependency}
					...
					\depedge{3}{5}{nmod}
				\end{dependency}
				\caption{\emph{Sempre} tivemos canções \emph{mais} lentas}
				\label{fig:advmod2}
			\end{figure*}
	
		\subsubsection{Caso seja nome}
			
			 O núcleo nominal de um adjunto adverbial (substantivo, pronome, sintagma nominal) será anotado como \emph{obl}.
			...
			A relação \emph{obl:agent} é utilizada para indicar o agente de um verbo na voz passiva. (ver Xx) Além disso, a relação de \emph{obl} também funciona como complemento de adjetivo. (ver Xx)

		\subsubsection{Caso seja verbo}
			A oração subordinada adverbial será anotada como \emph{advcl}, não havendo diferenciação semântica (temporal, causal, concessiva, final, etc).
		...
		\subsubsection{\emph{Teve uma relação extraconjugal com Monica Lewinsky}}
